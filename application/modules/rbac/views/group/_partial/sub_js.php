...
<script type="text/javascript">
    ...
    $(document).on('click', '#btn-add', function(event) {
        ...
        form.attr('action', "<?= site_url('/rbac/group/simpan/') ?>");

    	modal.find('.modal-title').text('Form Master Group');
    	modal.modal('show');
    });
    ...
    $(document).on('click', '.btn-edit', function(event) {
    	...
    	.done(function(data) {
    		if (data) {
	    		let form = $('#form-single');
	    		form.attr('action', "<?= site_url('/rbac/group/simpan/') ?>" + id);
	    		form.find('#id_label').val(data.label);
	    		form.find('#id_desc').val(data.desc);
    ...
    $(document).on('click', '.btn-delete', function(event) {
    	...
    	customConfirmation({type:'warning'}, (confirmation) => {
	    	if (confirmation) {
		    	$.ajax({
		    		url: "<?= site_url('/rbac/group/hapus/') ?>" + id,
		    		type: 'POST',
		    		dataType: 'json',
		    		data: {
		    			[csrf_name] : csrf_hash
